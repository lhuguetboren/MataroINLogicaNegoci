...
class BalconingException(HotelException):
    pass
...
class Habitacion:
    ...
    def __str__(self):
        return f"Habitación {self.numero} Suite {self.suite}- {'Libre' if self.libre else f'Ocupada por {self.usuario}'}"
...
class Hotel(Establecimiento):
    ...
    def reservar_suite(self, usuario: Usuario, cantidad: int = 1) -> bool:
        habitaciones_libres = [hab for hab in self.habitaciones if hab.libre and hab.suite]
        if len(habitaciones_libres) < cantidad:
            raise ReservaException("No hay suficientes habitaciones libres.")
        ...
        for i in range(cantidad):
            habitaciones_libres[i].reservar(usuario)
        self.usuarios.append(usuario)
        print(f"{cantidad} suite(es) reservada(s) en {self.nombre} por {usuario}.")
        return True
...
class HotelLujo(Hotel):
    def __init__(self, nombre: str, direccion: str, localidad: str, categoria: int, num_habitaciones: int, tiene_spa: bool, tiene_restaurante_gourmet: bool):
...
# Subclase HoteSuperlLujo que hereda de HotelLujo
class HoteSuperlLujo(HotelLujo):
    def __init__(self, nombre: str, direccion: str, localidad: str, categoria: int, num_habitaciones: int,num_suites: int,tiene_spa:bool, tiene_restaurante_gourmet:bool):
        super().__init__(nombre, direccion, localidad, categoria, num_habitaciones,tiene_spa, tiene_restaurante_gourmet)
...
if __name__ == "__main__":
    ...
    '''
    hotel6=HoteSuperlLujo("luxury suites", "rockefeller", "UncleDonald", 2, 80, 30, True, True)
    sistema.agregar_hotel(hotel6)

    try:
        usuario1 = Usuario("Juan Perez")
        usuario2 = Usuario("John Smith")
    except BalconingException as e:
        print("usuario sin acceso")
    
    try:
        hotel6.reservar_suite(usuario1, 1)
    ...
    try:
        hotel1.reservar_habitacion(usuario1, 2)
        hotel2.reservar_habitacion(usuario2, 1)
    except ReservaException as e:
        print(f"Error en la reserva: {e}")
